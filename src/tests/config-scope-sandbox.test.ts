...
describe.concurrent("config — scope integration", () => {
  it("default (no config): main and agent share the same sandbox", async () => {
    const { session, teardown } = await setupScenario("cc-msb-scope-default-", { fixture: "config-scope-default" });
    try {
  ...
  it("per-agent: main session uses the global sandbox image (ubuntu)", async () => {
    const { session, teardown } = await setupScenario("cc-msb-scope-pa-main-", { fixture: "config-scope-per-agent-image" });
    try {
      const result = await session.run(
  ...
  it("per-agent: test-agent subagent gets its own sandbox with its configured image (debian)", async () => {
    const { session, teardown } = await setupScenario("cc-msb-scope-pa-agent-", { fixture: "config-scope-per-agent-image" });
    try {
      const result = await session.run(
  ...
  it("session scope: file written in one bash call is readable in the next", async () => {
    const { session, teardown } = await setupScenario("cc-msb-scope-sess-", { fixture: "config-scope-session" });
    try {
      const result = await session.run(
  ...
  it("per-run scope: file written in one agent bash call is gone in the next", async () => {
    const { session, teardown } = await setupScenario("cc-msb-scope-perrun-", { fixture: "config-scope-per-run" });
    try {
      const result = await session.run(
