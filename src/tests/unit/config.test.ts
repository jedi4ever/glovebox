...
describe("config — secrets", () => {
  ...
  it("$VAR with unset host env var is silently skipped", () => {
    ...
    try {
      ...
      // The unset-VAR secret should not appear at all.
  ...
  it("agent-specific secrets are picked up for an agent call", () => {
    ...
    try {
      ...
      const args = readCreateArgs(PER_AGENT_SANDBOX);
      expect(args).toContain("AGENT_TOKEN=agent-secret@agent.example");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe("config — bare defaults.<key> (shared baseline)", () => {
  ...
  it("defaults.agents.X beats defaults.X for agents", () => {
    ...
    try {
      runHook(dir, {}, "test-agent");
      // defaults.agents.sandbox_image=alpine is more specific than defaults.sandbox_image=debian
      expect(readCreateArgs(PER_AGENT_SANDBOX)[0]).toBe("alpine");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("parser ignores keys nested under defaults.main/defaults.agents when reading defaults.X", () => {
    // The bare `defaults.sandbox_image` is absent; only nested ones exist.
    // The parser must NOT accidentally pick those up as `defaults.sandbox_image`.
    const dir = makeLocalConfig(
      "defaults:\n" +
      "  main:\n" +
      "    sandbox_image: debian\n" +
    ...
    try {
      // main session: should resolve to debian via defaults.main, not via accidental defaults
      runHook(dir);
      expect(readCreateArgs()[0]).toBe("debian");
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  it("defaults.network accepts a YAML list", () => {
    const dir = makeLocalConfig(
      ...
      "    - two.example.com\n" +
      "  main:\n" +
      "    sandbox_image: ubuntu\n"   // sibling sub-mapping right after the list
    );
    try {
      runHook(dir);
      const args = readCreateArgs();
  ...
  it("defaults.sandbox_image: alpine overrides the built-in ubuntu for both main and agents", () => {
    ...
    try {
      // Main session — built-in default ubuntu is replaced by alpine
      runHook(dir);
      expect(readCreateArgs()[0]).toBe("alpine");
      expect(readCreateArgs()[0]).not.toBe("ubuntu");

      // Agent — same shared default applies
      cleanupFakeMsbFiles();
      runHook(dir, {}, "test-agent");
      expect(readCreateArgs(PER_AGENT_SANDBOX)[0]).toBe("alpine");
    } finally {
...
describe("config — defaults.main block", () => {
  ...
  it("defaults.main does NOT apply to agents", () => {
    ...
    try {
      runHook(dir, {}, "test-agent");
      // Agent ignores defaults.main → falls through to defaults.agents.sandbox_image
      expect(readCreateArgs(PER_AGENT_SANDBOX)[0]).toBe("alpine");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("defaults.main works for network setting", () => {
    ...
    try {
      runHook(dir);
      // main inherits defaults.main.network=disabled
      expect(readCreateArgs()).toContain("--no-net");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
