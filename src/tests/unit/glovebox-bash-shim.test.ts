...
const RUNNING_SANDBOX = "glovebox-shim-test-001";
...
function cleanupFakeState() {
  readdirSync("/tmp")
    .filter((f) => f.startsWith("fake-msb-glovebox-shim-test-"))
...
describe("glovebox-bash.mjs — shell shim", () => {
  it("with NO running glovebox sandbox: falls back to /bin/bash for -c <cmd>", () => {
    const r = runShim(["-c", "echo shim-fallback-ok"]);
    expect(r.status).toBe(0);
    expect(r.stdout.trim()).toBe("shim-fallback-ok");
  });

  it("with a running glovebox sandbox: routes through msb exec", () => {
  ...
  it("already-wrapped command (looks like the PreToolUse rewrite) is NOT double-wrapped", () => {
    writeFileSync(`/tmp/fake-msb-${RUNNING_SANDBOX}.state`, "Running");
    // Simulate what glovebox's Bash hook produces — a string containing `| msb exec ... -- bash`.
    // The shim should detect this and pass through to real bash unchanged.
    const wrapped = "printf '%s' 'ZWNobyBkb3VibGUtd3JhcC1jaGVjaw==' | base64 -d | msb exec 'glovebox-shim-test-001' -- bash";
  ...
  it("zero-arg invocation defers to host bash (interactive form)", () => {
    ...
    // Pass `true` (resolved via PATH) via GLOVEBOX_HOST_BASH so the zero-arg branch exits cleanly.
    const r = spawnSync("node", [SHIM], {
      encoding: "utf8",
      env: { ...process.env, PATH: `${FAKE_MSB_DIR}:${process.env["PATH"]}`, GLOVEBOX_HOST_BASH: "true" },
    });
    expect(r.status).toBe(0);
  });
