#!/usr/bin/env node
// Resolves the effective glovebox configuration for a project and agent.
...
export async function resolveConfig(projectDir, agentType = '') {
  const localFile = join(projectDir, '.glovebox.yml');
  const globalConfigDir = process.env.GLOVEBOX_CONFIG_DIR
    || join(process.env.HOME || '~', '.config/glovebox');
  ...
  const presetNames = (() => {
    if (process.env.GLOVEBOX_PRESETS) {
      return process.env.GLOVEBOX_PRESETS.split(',').map(s => s.trim()).filter(Boolean);
    }
  ...
  const userPresetsDir    = process.env.GLOVEBOX_PRESETS_DIR || join(process.env.HOME || '~', '.glovebox/presets');
