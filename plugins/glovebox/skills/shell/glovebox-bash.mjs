#!/usr/bin/env node
// glovebox-bash.mjs — drop-in shim for /bin/bash.
//
// When invoked as `glovebox-bash.mjs -c <cmd>`, forwards <cmd> into the
// current Claude Code session's glovebox MSB sandbox via `msb exec`.
...
const hostBash = process.env.GLOVEBOX_HOST_BASH || '/bin/bash';
const traceLog = process.env.GLOVEBOX_SHELL_TRACE_LOG;
...
// Already a glovebox rewrite → avoid double-wrapping
if (cmd && /\| msb exec .*-- bash/.test(cmd)) fallback(['-c', cmd]);

// Find newest running glovebox-* sandbox
const listResult = spawnSync('msb', ['list'], { encoding: 'utf8' });
let sandbox = '';
if (!listResult.error) {
  for (const line of (listResult.stdout || '').split('\n')) {
    if (/^glovebox-/.test(line) && /running/i.test(line)) {
