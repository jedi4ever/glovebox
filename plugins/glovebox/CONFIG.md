# `.glovebox.yml` configuration reference

Two config files are consulted on every tool call, in priority order:

1. **Local** — `$CLAUDE_PROJECT_DIR/.glovebox.yml` (project root)
2. **Global** — `${GLOVEBOX_CONFIG_DIR:-$HOME/.config/glovebox}/config.yml`
...
Set `GLOVEBOX_CONFIG_DIR` to point the global file somewhere else (useful for testing, or for keeping your global config under a dotfiles repo).
...
1. Env var (`GLOVEBOX_MAIN_*` or the legacy `GLOVEBOX_SANDBOX_IMAGE`/`GLOVEBOX_SANDBOX_NAME`)
...
1. Env var `GLOVEBOX_AGENT_<SETTING>_<AGENT_NAME>` (agent name uppercased with `-` → `_`)
...
`sandbox_name` is the one exception to the chain above: it is per-context and never inherited from `defaults.agents`. Agents fall back to the main session's `sandbox_name` (or `GLOVEBOX_SANDBOX_NAME`) when their own isn't set. `sandbox_image` for agents has an extra step too — the legacy `GLOVEBOX_SANDBOX_IMAGE` env var is consulted between the agent's own config (in both files) and the `defaults.agents` block (in both files).
...
**Env override**: `GLOVEBOX_MAIN_SCOPE`, `GLOVEBOX_AGENT_SCOPE_<NAME>`
...
**Env override**: `GLOVEBOX_SANDBOX_IMAGE` (legacy, applies to main), `GLOVEBOX_AGENT_IMAGE_<NAME>`
...
**Env override**: `GLOVEBOX_SANDBOX_NAME` (main), `GLOVEBOX_AGENT_SANDBOX_NAME_<NAME>`
...
**Env override**: `GLOVEBOX_MAIN_MOUNT_WORKDIR`, `GLOVEBOX_AGENT_MOUNT_WORKDIR`
...
**Env override**: `GLOVEBOX_MAIN_PASS_ENV`, `GLOVEBOX_AGENT_PASS_ENV_<NAME>`
...
**Env override**: `GLOVEBOX_MAIN_NETWORK`, `GLOVEBOX_AGENT_NETWORK_<NAME>`
...
**Env override**: `GLOVEBOX_MAIN_PORTS`, `GLOVEBOX_AGENT_PORTS_<NAME>`
...
**Env override**: `GLOVEBOX_MAIN_SECRETS`, `GLOVEBOX_AGENT_SECRETS_<NAME>`
...
**Env override**: `GLOVEBOX_MAIN_ON_SECRET_VIOLATION`, `GLOVEBOX_AGENT_ON_SECRET_VIOLATION_<NAME>`
...
**Env overrides**: `GLOVEBOX_MAIN_TLS_INTERCEPT`, `GLOVEBOX_MAIN_TLS_INTERCEPT_PORT`, `GLOVEBOX_MAIN_TLS_BYPASS`, `GLOVEBOX_MAIN_TRUST_HOST_CAS` (and the `GLOVEBOX_AGENT_…_<NAME>` equivalents).
...
**Env overrides**: `GLOVEBOX_MAIN_GIT_USER_NAME`, `GLOVEBOX_MAIN_GIT_USER_EMAIL`, `GLOVEBOX_MAIN_GIT_USER_AUTODETECT`, plus the `GLOVEBOX_AGENT_…_<NAME>` equivalents.
...
**Env overrides**: `GLOVEBOX_MAIN_GITHUB_TOKEN`, `GLOVEBOX_MAIN_GITHUB_HOSTS`, `GLOVEBOX_MAIN_GIT_TOKEN_AUTODETECT`, plus the `GLOVEBOX_AGENT_…_<NAME>` equivalents.
...
**Env override**: `GLOVEBOX_MAIN_AUTO_RECREATE`, `GLOVEBOX_AGENT_AUTO_RECREATE_<NAME>`

### Config-drift detection (persistent sandboxes)

msb applies most settings (`sandbox_image`, `network`, `ports`, `secrets`, `on_secret_violation`, `tls_intercept`, `tls_intercept_port`, `tls_bypass`, `trust_host_cas`, `mount_workdir`) **only at `msb create` time**. They're baked into the sandbox's lifetime. If you edit `.glovebox.yml` (or `~/.config/glovebox/config.yml`) while a long-lived sandbox is up (e.g. `scope: named` / `scope: directory`), the running sandbox keeps using the old flags — silently.

To make this fail loudly, the plugin persists a 16-char fingerprint of every create-time setting to `~/.cache/glovebox/fingerprints/<sandbox>.fp` whenever it creates a sandbox. On every subsequent hook call, the current effective config is re-hashed and compared. If the fingerprints differ, the hook emits a **deny** with a one-line recreate instruction:

```
glovebox: settings in .glovebox.yml changed since sandbox 'my-project' was created.
...
The plugin denies the `WebFetch` tool with a hint telling Claude to use `Bash` with `curl` instead. The Bash hook then routes that curl call through `msb exec` so the fetch happens **inside** the sandbox and respects the configured `network` policy — otherwise CC's built-in WebFetch would bypass every glovebox config and hit the URL from the host.

PreToolUse hooks can't return synthetic tool results, so this two-turn bounce (deny → next-turn Bash) is the cleanest path. There's no opt-out per se; `scope: host` already bypasses every glovebox hook, including this one.
...
- Main settings use a fixed name: `GLOVEBOX_MAIN_<SETTING>` (e.g. `GLOVEBOX_MAIN_SCOPE`).
- Agent settings include the agent name uppercased with `-` mapped to `_`:
  `agent_type: test-agent` → `GLOVEBOX_AGENT_SCOPE_TEST_AGENT`
- Two legacy globals are still recognized for backwards compatibility:
  `GLOVEBOX_SANDBOX_IMAGE` (main image), `GLOVEBOX_SANDBOX_NAME` (main named sandbox).
...
In the common case — your machine has `git config --global user.name`/`user.email` set and you're logged in via `gh auth login` — **no config is needed at all**. The plugin autodetects both. Just leave the new sections out of your `.glovebox.yml`.
...
Put this in `~/.config/glovebox/config.yml` (or `$GLOVEBOX_CONFIG_DIR/config.yml`). Any project without its own `.glovebox.yml` inherits these settings; projects with a local file can selectively override.
