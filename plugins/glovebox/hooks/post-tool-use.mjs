...
if (existsSync(noticePath)) {
  ...
  const msg = notice.type === 'recreated'
    ? `glovebox: sandbox '${notice.sandbox}' was automatically recreated to apply updated settings. In-memory state has been reset; project files are intact.`
    : `glovebox: sandbox '${notice.sandbox}' was restarted (it was stopped). Ephemeral in-memory state has been reset; filesystem is preserved.`;
